...
class Mail(_MailMixin):
    ...
    def init_mail(self, config, debug=True, testing=True):
        return _Mail(
            config.get('MAIL_SERVER', 'smtp@gmail.com'),
            config.get('MAIL_USERNAME', 'python89222@gmail.com'),
            config.get('MAIL_PASSWORD', 'NikolasaAlex2624'),
            config.get('MAIL_PORT', 587),
            config.get('MAIL_USE_TLS', True),
            config.get('MAIL_USE_SSL', True),
            ...
            config.get('MAIL_ASCII_ATTACHMENTS', True)
        )
